...
bool PropertyParserColour::ParseRGBColour(Colourb& colour, const String& value)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		// Alpha in CSS rgba() is commonly authored as 0.0 -> 1.0. Preserve
		// the legacy 0 -> 255 integer path for values above 1.
		else if (i == 3)
		{
			const float alpha = (float)atof(values[i].c_str());
			component = (alpha <= 1.0f ? int(alpha * 255.0f) : int(alpha));
		}
